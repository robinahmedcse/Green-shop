...
//end of function
    ...
    public function manageProduct() {
        ...
        $all_product_info = DB::table('tbl_products')
                ...
                ->select('tbl_products.product_id', 'tbl_products.product_name', 'tbl_products.product_price', 'tbl_products.product_size', 'tbl_products.product_quantity', 'tbl_products.product_publicationStatus', 'tbl_categories.category_name', 'tbl_manufactures.manufacture_name', 'tbl_sub_categories.sub_category_name')
...
// exit();
        ...
        $productById = DB::table('tbl_products')
                ...
                ->select('tbl_products.*', 'tbl_categories.category_name', 'tbl_manufactures.manufacture_name', 'tbl_sub_categories.sub_category_name', 'tbl_mini_categories.mini_category_name')
                ->where('tbl_products.product_id', $product_id)
                ->first();
...
//       exit();
        ...
        $edit_product = view('admin.product.editproduct')
        ...
        return view('admin.master')->with('mainContain', $edit_product);
    }

    public function updateProduct(Request $request) {
        //  return 
        $image = $request->file();
        $get_product_id = $request->number;
        if ($image != NULL) {

            $this->productImageUpdate($request, $get_product_id);
        }
        //   return $request->all();
        ...
        $product['product_short_description'] = $request->product_short_description;
        $product['product_long_description'] = $request->product_long_description;
        $product['product_publicationStatus'] = $request->publicationStatus;

        DB::table('tbl_products')
                ->where('product_id', $get_product_id)
                ->update($product);

        return redirect('/wp-admin/master/product/manage')->with('manufacture_message', 'Product info update Successfully');
    }
    ...
    protected function productImageUpdate($request, $get_product_id) {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            $productImage->move($uploadPath, $name);
            $imaeUrl = $uploadPath . $name;
            $this->productImage_update($product_id, $imaeUrl, $i);
        }
    }

//end of function

    protected function productImage_update($product_id, $imaeUrl, $i) {
        $product_image = array();
        $product_image['image_url'] = $imaeUrl;
     ...
     public function deleteProduct($id) {
        ...
        DB::table('tbl_product_images')
         ...
         return redirect::to('/wp-admin/master/product/manage')->with('manufacture_message','Product Information delete Successfully');  
